...
| [Using 3rd Party Containers](./04-3rd-party-containers/using-3rd-party-containers.md)
...
- **docker run** : Instructs Docker to create and start a new container from a specific image.
- **-d (Detached Mode)** : Runs the container in the background, freeing up the current terminal window immediately.
- **--rm (Auto-Remove)** : Deletes the container's file system automatically when it stops. This prevents dead, unused containers from cluttering the disk spaces.
- **-v pgdata:/var/lib/postgresql/data (Volume Mount)** : Creates or attaches a persistant Docker volume named pgdata. It links it to the database directory inside the container. Because of this, the actual database data is safely preserved on the computer even after the container is destroyed by **--rm**.
- **-e POSTGRES_PASSWORD=foobarbaz (Evn Var)** : Sets the required master password for the default database administrator user (postgres) to foobarbaz.
- **-p 5432:5432 (Port Mapping)** : Forwards traffic from 5432 on our localhost machine to port 5432 inside the container. This allows external apps or GUI clients on our computer to connect to the database via *localhost:5432*.
- **postgres:15.1-alpine (The Image)** : Specifies the exact software to run. It uses PostgreSQL version 51.1 built on Alpine Linux, a minimalist operating system designed to keep the final container size exceptionally small. 

- **-v ${PWD}/postgres.conf:/etc/postgresql/postgresql.conf (Bind Mount)** : PWD is nothing but the current working directory.
This maps a file named postgres.conf from the current folder directly into the container at /etc/postgresql/postgresql.conf.
Any changes done to this local file will reflect instantly inside the container.
- **-c 'config_file=/etc/postgresql/postgresql.conf' (PostgreSQL Flag)** : Arguments placed after the image (postgres:15.1=alpine) are passed directly as startup commands to PostgreSQL. The **-c** flag tells the PostgreSQL engine to override a configuring parameter.
- **config_file=...** explicitly directs the database to boot up using the custom file just mounted.

- **NOTE** : For this command to work successfully, a file named **postgres.conf** must already exist in your current terminal folder before running the command. If it doesn't, Docker might accidentally create an empty directory named postgres.conf instead of mounting a file, causing the database to crash on startup.
